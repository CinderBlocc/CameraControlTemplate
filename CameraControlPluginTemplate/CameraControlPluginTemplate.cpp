...
{
	Initialize();
	cvarManager->registerNotifier("EnableCamControlTemplatePlugin", [this](std::vector<std::string> params){Enable();}, "Enables camera control plugin", PERMISSION_ALL);
	cvarManager->registerNotifier("DisableCamControlTemplatePlugin", [this](std::vector<std::string> params){Disable();}, "Disables camera control plugin", PERMISSION_ALL);
}
...
void CameraControlPluginTemplate::CreateValues()
{
	...
	*/
	...
	FOCUS.Y = 0;
	FOCUS.Z = 250;
	ROTATION.Pitch = -20 * 182.044449 - SWIVEL.Pitch;
	ROTATION.Yaw = 0;
	ROTATION.Roll = 0;
	DISTANCE = 200;
	FOV = 90;
...
bool CameraControlPluginTemplate::IsCVarNull(std::string cvarName)
{
